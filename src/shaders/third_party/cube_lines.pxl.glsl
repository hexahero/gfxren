...
vec3 fcos(vec3 x) {
    ...
    //w*=0.; //test
    float lw=length(w);
    if((lw==0.)||isnan(lw)||isinf(lw)){vec3 tc=vec3(0.); for(int i=0;i<8;i++)tc+=cos(x+x*float(i-4)*(0.01*400./screenRes.y));return tc/8.;}
...
#ifdef ANIM_SHAPE
    float curvature = (0.001+1.5-1.5*smoothstep(0.,8.5,mod((time+tshift)*0.44,20.))*(1.-smoothstep(10.,18.5,mod((time+tshift)*0.44,20.))));
...
void main()
{
...
#ifdef MOUSE_control
    mouseY = (1.0 - 1.15 * mousePos.y / screenRes.y) * 0.5 * PI;
    if(mousePos.y < 1.)
#endif
#ifdef CAMERA_POS
    ...
    float mouseX = -2.*PI-0.25*(time*ROTATION_SPEED+tshift);
#else
    float mouseX = -2.*PI-0.25*(time+tshift);
#endif
#ifdef MOUSE_control
    mouseX+=-(mousePos.x / screenRes.x) * 2. * PI;
#endif
    ...
    vec2 uv = (gl_FragCoord.xy + o - 0.5 * screenRes.xy) / screenRes.x;
#else
    vec2 uv = (gl_FragCoord.xy - 0.5 * screenRes.xy) / screenRes.x;
#endif
...
#if defined(BG_ALPHA)||defined(ONLY_BOX)||defined(SHADOW_ALPHA)
    FragColor.rgb=FragColor.rgb*FragColor.w+texture(iChannel0, fragCoord/screenRes.xy).rgb*(1.-FragColor.w);
#endif
